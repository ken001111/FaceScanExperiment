...
# torch>=2 enforces same-device ops: move octlevel to scene_extent's device
python - <<'PY'
...
for repo in ("~/svraster", "~/geosvr"):
    ...
    if not p.exists():
        continue
    s = p.read_text()
    old = "def level_2_vox_size(scene_extent, octlevel):\n    return torch.ldexp(scene_extent, -octlevel)"
    new = ("def level_2_vox_size(scene_extent, octlevel):\n"
    ...
    elif "same-device requirement" in s:
        print(f"already patched {p}")
    else:
        print(f"PATTERN NOT FOUND in {p} — inspect manually")
PY
