source ~/miniconda3/etc/profile.d/conda.sh; conda activate facescan
python - <<'PY'
import open3d as o3d, numpy as np, os
CMP="/mnt/c/Users/m352395/Downloads/mesh_compare"; OUT="/mnt/c/Users/m352395/Downloads/mesh_previews"
m=o3d.io.read_triangle_mesh(f"{CMP}/sugar_refined_cropped.ply")
# keep components above 200 tris (drop small floaters), then render
c=m.cluster_connected_triangles(); tc,nt=np.asarray(c[0]),np.asarray(c[1])
keep=np.isin(tc, np.where(nt>=200)[0]); m.remove_triangles_by_mask(~keep); m.remove_unreferenced_vertices()
m.compute_vertex_normals()
e=np.asarray(m.get_axis_aligned_bounding_box().get_extent())
print(f"refined cropped (cleaned): {len(m.vertices)} verts, {len(m.triangles)} tris, extent {np.round(e,1)}")
o3d.io.write_triangle_mesh(f"{CMP}/sugar_refined_cropped.ply", m, write_vertex_colors=False)
ctr=m.get_axis_aligned_bounding_box().get_center(); rad=np.linalg.norm(e)*0.9
r=o3d.visualization.rendering.OffscreenRenderer(900,900)
mat=o3d.visualization.rendering.MaterialRecord(); mat.shader="defaultLit"; mat.base_color=[0.75,0.75,0.78,1.0]
r.scene.add_geometry("m",m,mat); r.scene.set_background([1,1,1,1])
r.scene.scene.set_sun_light([0.3,-0.4,-0.6],[1,1,1],90000); r.scene.scene.enable_sun_light(True)
for vn,d in {"front":(0,0,1),"threequarter":(0.7,0.3,0.7),"left":(1,0,0)}.items():
    d=np.asarray(d,float); d/=np.linalg.norm(d); eye=ctr+d*rad*2.2
    r.setup_camera(50.0,ctr,eye,[0,1,0]); o3d.io.write_image(f"{OUT}/sugar_refined_crop_{vn}.png", r.render_to_image())
print("rendered cropped refined views")
PY
