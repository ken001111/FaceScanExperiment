...
echo "[refine] scale + render + add to compare"
python - <<'PY'
import open3d as o3d, numpy as np, glob, os, shutil
...
m=o3d.io.read_triangle_mesh(sorted(objs)[-1])
...
# cropped to 2dgs head bbox for a clean face view
...
# render
for nm,mesh in [("sugar_refined", m)]:
    mesh.compute_vertex_normals()
    ctr=mesh.get_axis_aligned_bounding_box().get_center(); rad=np.linalg.norm(mesh.get_axis_aligned_bounding_box().get_extent())*0.9
    ...
    for vn,d in {"front":(0,0,1),"threequarter":(0.7,0.3,0.7)}.items():
        ...
        r.setup_camera(50.0,ctr,eye,[0,1,0]); o3d.io.write_image(f"{OUT}/{nm}_{vn}.png", r.render_to_image())
print("rendered sugar_refined views + added to mesh_compare")
PY
echo "DONE_REFINE_SUGAR"
