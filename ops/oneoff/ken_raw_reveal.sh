...
print("head center:", np.round(ctr,2))
p = os.path.expanduser("~/FaceScan/param_study/geosvr_ken_raw/mesh/tsdf/tsdf_fusion_post.ply")
...
print("after crop:", len(m.vertices))
...
for i in range(4):
    ...
    o3d.io.write_image(f"{OUT}/kenraw_{i}.png", r.render_to_image())
o3d.io.write_triangle_mesh(os.path.expanduser("~/geosvr_ken_raw_head.ply"), m)
print("KEN RAW REVEAL DONE")
PY
