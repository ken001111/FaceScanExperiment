...
pip install -q PyMCubes 2>&1 | tail -1
python -c "import mcubes; print('mcubes OK')" || { echo "mcubes install failed"; exit 1; }
...
echo "exit=$?"
grep -iE "marching|Mesh saved|Vertices|Cleaning|Traceback|Error|mcubes" output/mc_extract.log | tail -12
