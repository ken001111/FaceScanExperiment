...
d8 = obs_tree.query(g_all, k=8, workers=-1)[0][:, -1]
vis = d8 < 0.10   # >=8 lidar returns within 10cm = densely observed
...
for arm in ["nodepth", "mono", "lidar", "lidarconf", "fused"]:
    ...
    r["mesh"] = mesh_path
    json.dump(r, open(f"{ABLATION}/{arm}/eval_vs_laser_v5.json", "w"), indent=1, default=float)
